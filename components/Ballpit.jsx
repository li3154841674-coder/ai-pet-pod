...
class W {
  ...
  update(e) {
    ...
    // 获取动态重力值
    const gravityX = t.gravityX || 0;
    const gravityY = t.gravityY || 0;
    const baseGravity = t.gravity || 0.5;
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      // 应用重力：结合基础重力和设备倾斜
      B.y -= e.delta * (baseGravity + gravityY * 2) * n[idx];
      B.x += e.delta * gravityX * 2 * n[idx];
...
const Ballpit = ({ className = '', followCursor = false, ...props }) => {
  ...
  const lastGravityX = useRef(0);
  const lastGravityY = useRef(0);
  ...
  useEffect(() => {
    ...
    // 环境诊断
    console.log('🎾 Ballpit: 初始化中...');
    console.log('🎾 DeviceOrientationEvent 支持:', typeof DeviceOrientationEvent !== 'undefined');
    if (typeof DeviceOrientationEvent !== 'undefined') {
      console.log('🎾 requestPermission 支持:', typeof DeviceOrientationEvent.requestPermission === 'function');
    }
    ...
    const handleScroll = () => {
      ...
      if (scrollDelta > 10 && spheresInstanceRef.current) {
        const spheres = spheresInstanceRef.current.spheres;
        if (spheres && spheres.physics) {
          const { velocityData, count } = spheres.physics;
          for (let i = 0; i < count; i++) {
            const base = 3 * i;
            velocityData[base] += (Math.random() - 0.5) * 0.1 * (scrollDelta / 100);
            velocityData[base + 1] += (Math.random() - 0.5) * 0.1 * (scrollDelta / 100);
          }
        }
      }
    ...
    const handleOrientation = (event) => {
      console.log('🎾 收到 deviceorientation 事件:', {
        beta: event.beta,
        gamma: event.gamma,
        alpha: event.alpha
      });
      ...
      const spheres = spheresInstanceRef.current.spheres;
      if (!spheres || !spheres.physics) return;
      ...
      // beta > 0: 向下倾斜 → 球向下
      // beta < 0: 向上倾斜 → 球向上
      const gravityX = (gamma / 45) * 1.0;
      const gravityY = (beta / 45) * 1.0;

      // 平滑过渡
      lastGravityX.current = lastGravityX.current * (1 - smoothFactor) + gravityX * smoothFactor;
      lastGravityY.current = lastGravityY.current * (1 - smoothFactor) + gravityY * smoothFactor;

      // 更新物理系统
      spheres.config.gravityX = lastGravityX.current;
      spheres.config.gravityY = lastGravityY.current;
      
      console.log('🎾 应用重力:', { 
        gravityX: lastGravityX.current, 
        gravityY: lastGravityY.current 
      });
    };
    ...
    const requestPermission = async () => {
      ...
      if (typeof DeviceOrientationEvent !== 'undefined' && 
        ...
        try {
          console.log('🎾 调用 requestPermission()...');
          const permission = await DeviceOrientationEvent.requestPermission();
          console.log('🎾 权限结果:', permission);
          
          if (permission === 'granted' && !orientationListenerActive.current) {
            console.log('🎾 添加 deviceorientation 监听器');
            window.addEventListener('deviceorientation', handleOrientation);
            orientationListenerActive.current = true;
          }
        } catch (error) {
          console.error('🎾 Device orientation 权限请求失败:', error);
        }
      } else if (typeof DeviceOrientationEvent !== 'undefined' && !orientationListenerActive.current) {
        console.log('🎾 非 iOS 设备，直接添加监听器');
        window.addEventListener('deviceorientation', handleOrientation);
        orientationListenerActive.current = true;
      } else {
        console.warn('🎾 DeviceOrientationEvent 不支持');
      }
    };

    // 在用户首次交互时请求权限
    const handleFirstInteraction = () => {
      console.log('🎾 用户首次交互，请求权限');
      requestPermission();
    ...
    if (typeof DeviceOrientationEvent !== 'undefined' && 
        typeof DeviceOrientationEvent.requestPermission !== 'function' && 
        !orientationListenerActive.current) {
      console.log('🎾 立即添加 deviceorientation 监听器');
      window.addEventListener('deviceorientation', handleOrientation);
      orientationListenerActive.current = true;
    }
  ...
  return <canvas className={className} ref={canvasRef} style={{ width: '100%', height: '100%', touchAction: 'none !important' }} />;
};
